...
THE PROBLEM (visible in nsys cudaapisum):
  ...
  iteration (up to 20 000 per building). The nsys trace shows thousands of
  tiny DtoH calls with the GPU sitting idle between each one.
  This matches exactly what the Week 10 slides show as the main pitfall
  when mixing GPU computation with CPU-side Python control flow.

THE FIX:
  Only call delta.item() (= GPU sync) every CHECK_FREQ=100 iterations.
  The GPU runs 100 kernels asynchronously before the CPU checks convergence,
  keeping the SM utilisation high.
...
MAX_ITER   = 20_000
ABS_TOL    = 1e-4
CHECK_FREQ = 100    # ← key change: sync GPU→CPU only every 100 iterations
N_TOTAL    = 4571
...
_w = cp.zeros(1); _w + _w; cp.cuda.Stream.null.synchronize(); del _w  # warm-up
